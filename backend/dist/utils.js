...
const ObjectId_1 = require("./entities/ObjectId");
/**
 ...
 */
function merge(entity, data) {
    // should keep existing relations
    for (const [key, value] of Object.entries(data)) {
        if (Array.isArray(value) &&
            value.length > 0 &&
            value[0] instanceof ObjectId_1.ObjectId) {
            if (!(key in entity)) {
                throw new Error(`missing key ${key} in your entity, did you forgot to fetch your relation?`);
            }
            if (Array.isArray(entity[key])) {
                data[key] = data[key].map((entry) => {
                    const existingEntry = entity[key].find((entityEntry) => entityEntry.id == entry.id);
